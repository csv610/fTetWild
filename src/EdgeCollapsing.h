...
/// Perform edge collapsing operations on mesh
/// @param mesh Mesh to modify
/// @param tree AABB tree for spatial queries
void edge_collapsing(Mesh& mesh, const AABBWrapper& tree);

/// Collapse a single edge between two vertices
/// @return Number of tetrahedra removed
...
/// Check if edge is frozen (cannot be modified)
bool is_edge_freezed(Mesh& mesh, int v1_id, int v2_id);
/// Check if edge can be collapsed (within bounding box)
bool is_collapsable_bbox(Mesh& mesh, int v1_id, int v2_id);
/// Check if edge can be collapsed (length threshold)
bool is_collapsable_length(Mesh& mesh, int v1_id, int v2_id, Scalar l_2);
/// Check if edge can be collapsed (boundary constraints)
