...
//                pausee();
        ...
        if (!is_just_after_update) {
            if (max_energy - new_max_energy < constants::kEnergyConvergenceThreshold &&
                (avg_energy - new_avg_energy) / avg_energy < constants::kAvgEnergyConvergenceThreshold) {
...
//    }
    ...
    if (!mesh.is_input_all_inserted) {
        throw InputError("Input mesh not fully inserted - possible envelope issue");
...
//    C.conservativeResize(index, 3);
    ...
    for (int t_id = 0; t_id < mesh.tets.size(); ++t_id) {
        ...
        if (t.is_removed)
            continue;
        if (W(index) <= constants::kWeightThreshold) {
            t.is_removed = true;
        } else
            n_tets++;
        index++;
...
//    static const Scalar zero_area = 1e-10;
    ...
    for (int t_id = 0; t_id < tets.size(); t_id++) {
        ...
        if (t.is_removed)
            continue;
//        if (t.quality < 1e7)
        if (t.quality < constants::kQualityThreshold)
            continue;
